  ...
  (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if (req.body.role === 'guest') {
        res.render('entryadded', { warning: 'Non-registered users can not add data...', request: req.body });
      } else {
...
exports.entry_delete_post = (req, res) => {
  ...
  async.parallel(
    ...
    function(err, results) {
      ...
      if (req.body.role === 'guest') {
        res.render('entrydeleted', { warning: 'Non-registered users can not remove data...', request: req.body });
      } else {
    ...
    if (!errors.isEmpty()) {
      ...
      async.parallel(
        ...
        function(err, results) {
          ...
          if (req.body.role === 'guest') {
            res.render('entryupdated', { warning: 'Non-registered users can not update data...', request: req.body });
          } else {
            // Successful, so continue.
